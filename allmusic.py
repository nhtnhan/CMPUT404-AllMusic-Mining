...
res = soup.find_all("li", class_="artist")
ahref = res[0].findAll("div",class_="name")[0].a
name = ahref.text.strip()
link = ahref.attrs["href"]
related = link + '/related'
rfd = GET(related)
related_content = rfd.read()
soup = bs4.BeautifulSoup(related_content,features="html.parser")
res = soup.findAll("section",{"class":"related similars"})
